...
import numpy as np
import scipy.stats
from numpy import array, where, zeros, sqrt, linspace, sinh, exp, concatenate, \
    flip, ceil, nan, zeros_like, empty, unique, log10
from scipy.constants import Boltzmann as k_b, elementary_charge as e, pi, \
    electron_mass as m_e, h, epsilon_0
...
class Experiment:
    ...
    def __init__(self,
        ...
        self.backup_filename = os.path.join(filename, "bak")
    ...
    def generate_filename(self):
        return (f"{self.date.strftime('%Y-%m-%d %H.%M.%S')} - {self.chip} - "
                f"{self.device} - {self.experiment_name}.data")


class Settings:
    """ A class to store the experiment settings. By default, empty. """
    pass


class Thermoelectrics:
    class Calibration:
        """ Experiment data class for field-effect thermoelectric device
        calibration. """
                 ...
                 "dr": {"h1": [
                     ...
                     for y1 in i_h] if h == 1 else None,
                        "h2": [{"i_h": y2,
                                ...
                                "iv2": IV() if th == 0 or th == 2 else None}
                               for y2 in
                               i_h] if h == 2 else None} if x in t_h else None}
                for x in t]

            vt_samples = int(ceil((settings.adc.vt_settling_time + settings.adc.vt_measurement_time) / (settings.adc.n_plc / settings.adc.line_freq)))

            for idx_x, x in enumerate(self.t):
                if settings.tc.address is not None:
                    if idx_x == 0:
                        x["tt"].time = zeros(int(ceil(
                            settings.tc.settling_time_init * settings.tc.sampling_freq)))
                        x["tt"].stage = zeros(int(ceil(
                            settings.tc.settling_time_init * settings.tc.sampling_freq)))
                        x["tt"].shield = zeros(int(ceil(
                            settings.tc.settling_time_init * settings.tc.sampling_freq)))
                    if idx_x != 0:
                        x["tt"].time = zeros(int(ceil(
                            settings.tc.settling_time * settings.tc.sampling_freq)))
                        x["tt"].stage = zeros(int(ceil(
                            settings.tc.settling_time * settings.tc.sampling_freq)))
                        x["tt"].shield = zeros(int(ceil(
                            settings.tc.settling_time * settings.tc.sampling_freq)))
        ...
        def calculate_temperatures(drdc, drdc_err, drx, drx_err, dry,
                                   dry_err,
                                   fit):
        ...
        def __init__(self, h, th, t, i_h, f, settings):
            ...
            self.t = [{"t": x,
                       ...
                       "dr": {"h1": [[{"i_h": y1,
                                       ...
                                       "drt1": Lockin() if th == 0 or th == 1 else None,
                                       "drt2": Lockin() if th == 0 or th == 2 else None}
                                      for y2 in f] for y1 in
                                     i_h] if h == 1 else None,
                              "h2": [[{"i_h": y1,
                                       "f": y2,
                                       "drt1": Lockin() if th == 0 or th == 1 else None,
                                       "drt2": Lockin() if th == 0 or th == 2 else None}
                                      for y2 in f] for y1 in
                                     i_h] if h == 2 else None}}
                      for x in t]
    ...
    class PlotCalibration:
        ...
        def __init__(self, t, t_h, i_th, i_h, nrows=30, ncols=4,
                     wait=0.001):

            self.wait = wait
            self.grid = GridSpec(nrows, ncols)
            self.fig = plt.figure(figsize=(45 / 2.54, 25 / 2.54))
            self.fig.subplots_adjust(top=0.96, bottom=0.06, left=0.05,
                                     right=0.98, hspace=1.0, wspace=0.3)
            self.norm = Normalize(min(t), max(t))
            self.xlim_i = [i_th.min() - (i_th.max() - i_th.min()) * 0.1,
                           i_th.max() + (i_th.max() - i_th.min()) * 0.1]
            self.xlim_t = [t.min() - (t.max() - t.min()) * 0.1,
                           t.max() + (t.max() - t.min()) * 0.1]
            self.xlim_h = [
                1e3 * (i_h.min() - (i_h.max() - i_h.min()) * 0.1),
                1e3 * (i_h.max() + (i_h.max() - i_h.min()) * 0.1)]
            ...
            self.iv1.set_title("Thermometer 1 - $R(T)$", fontsize=10,
                               fontstyle="italic")
            self.iv1.set_xlim(self.xlim_i)
            # self.iv1.ticklabel_format(axis="y", style="scientific", scilimits=(0, 0))
            for idx, val in enumerate(t):
                self.iv1.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=val, marker="o",
                                         markeredgecolor='black',
                                         markeredgewidth=0.2, alpha=0.4))

            self.rt1 = self.fig.add_subplot(self.grid[12:24, 0],
                                            label="Th1")
            ...
            for idx, val in enumerate(t):
                self.rt1.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=val, marker="o",
                                         markeredgecolor='black',
                                         markeredgewidth=0.2, alpha=0.4))

            self.rterr1 = self.fig.add_subplot(self.grid[24:30, 0],
                                               label="Th1")
            self.rterr1.set_xlabel("T (K)")
            self.rterr1.set_ylabel(r"$\delta$ ($\Omega$)")
            self.rterr1.set_xlim(self.xlim_t)
            for idx, val in enumerate(t):
                self.rterr1.add_line(Line2D(xdata=[None], ydata=[None],
                                            color=self.cm(self.norm(val)),
                                            linewidth=0, label=val,
                                            marker="o",
                                            markeredgecolor='black',
                                            markeredgewidth=0.2, alpha=0.4))
            ...
            self.drx1.set_title("Thermometer 1 - $R(I_h)$", fontsize=10,
                                fontstyle="italic")
            for idx, val in enumerate(t_h):
                self.drx1.add_line(Line2D(xdata=[None], ydata=[None],
                                          color=self.cm(self.norm(val)),
                                          linewidth=0, label=val,
                                          marker="o",
                                          markeredgecolor='black',
            ...
            self.drxerr1 = self.fig.add_subplot(self.grid[7:10, 1],
                                                label="Th1")
            ...
            for idx, val in enumerate(t_h):
                self.drxerr1.add_line(Line2D(xdata=[None], ydata=[None],
                                             color=self.cm(self.norm(val)),
                                             linewidth=0, label=val,
                                             marker="o",
                                             markeredgecolor='black',
            ...
            self.dry1 = self.fig.add_subplot(self.grid[10:17, 1],
                                             label="Th1")
            ...
            for idx, val in enumerate(t_h):
                self.dry1.add_line(Line2D(xdata=[None], ydata=[None],
                                          color=self.cm(self.norm(val)),
                                          linewidth=0, label=val,
                                          marker="o",
                                          markeredgecolor='black',
            ...
            self.dryerr1 = self.fig.add_subplot(self.grid[17:20, 1],
                                                label="Th1")
            ...
            for idx, val in enumerate(t_h):
                self.dryerr1.add_line(Line2D(xdata=[None], ydata=[None],
                                             color=self.cm(self.norm(val)),
                                             linewidth=0, label=val,
                                             marker="o",
                                             markeredgecolor='black',
            ...
            self.drdc1 = self.fig.add_subplot(self.grid[20:27, 1],
                                              label="Th1")
            ...
            for idx, val in enumerate(t_h):
                self.drdc1.add_line(Line2D(xdata=[None], ydata=[None],
                                           color=self.cm(self.norm(val)),
                                           linewidth=0, label=val,
                                           marker="o",
                                           markeredgecolor='black',
            ...
            self.drdcerr1 = self.fig.add_subplot(self.grid[27:30, 1],
                                                 label="Th1")
            self.drdcerr1.set_xlabel("$I_h$ (mA)")
            self.drdcerr1.set_ylabel(r"$\delta$ ($\Omega$)")
            self.drdcerr1.set_xlim(self.xlim_h)
            for idx, val in enumerate(t_h):
                self.drdcerr1.add_line(Line2D(xdata=[None], ydata=[None],
                                              color=self.cm(self.norm(val)),
                                              linewidth=0, label=val,
                                              marker="o",
                                              markeredgecolor='black',
            ...
            self.iv2.set_title("Thermometer 2 - $R(T)$", fontsize=10,
                               fontstyle="italic")
            self.iv2.set_xlim(self.xlim_i)
            # self.iv2.ticklabel_format(axis="y", style="scientific", scilimits=(0, 0))
            for idx, val in enumerate(t):
                self.iv2.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=val, marker="o",
                                         markeredgecolor='black',
                                         markeredgewidth=0.2, alpha=0.4))

            self.rt2 = self.fig.add_subplot(self.grid[12:24, 2],
                                            label="Th2")
            ...
            for idx, val in enumerate(t):
                self.rt2.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=val, marker="o",
                                         markeredgecolor='black',
                                         markeredgewidth=0.2, alpha=0.4))

            self.rterr2 = self.fig.add_subplot(self.grid[24:30, 2],
                                               label="Th2")
            self.rterr2.set_xlabel("T (K)")
            self.rterr2.set_ylabel(r"$\delta$ ($\Omega$)")
            self.rterr2.set_xlim(self.xlim_t)
            for idx, val in enumerate(t):
                self.rterr2.add_line(Line2D(xdata=[None], ydata=[None],
                                            color=self.cm(self.norm(val)),
                                            linewidth=0, label=val,
                                            marker="o",
                                            markeredgecolor='black',
            ...
            self.drx2.set_title("Thermometer 2 - $R(I_h)$", fontsize=10,
                                fontstyle="italic")
            for idx, val in enumerate(t_h):
                self.drx2.add_line(Line2D(xdata=[None], ydata=[None],
                                          color=self.cm(self.norm(val)),
                                          linewidth=0, label=val,
                                          marker="o",
                                          markeredgecolor='black',
            ...
            self.drxerr2 = self.fig.add_subplot(self.grid[7:10, 3],
                                                label="Th2")
            ...
            for idx, val in enumerate(t_h):
                self.drxerr2.add_line(Line2D(xdata=[None], ydata=[None],
                                             color=self.cm(self.norm(val)),
                                             linewidth=0, label=val,
                                             marker="o",
                                             markeredgecolor='black',
            ...
            self.dry2 = self.fig.add_subplot(self.grid[10:17, 3],
                                             label="Th2")
            ...
            for idx, val in enumerate(t_h):
                self.dry2.add_line(Line2D(xdata=[None], ydata=[None],
                                          color=self.cm(self.norm(val)),
                                          linewidth=0, label=val,
                                          marker="o",
                                          markeredgecolor='black',
            ...
            self.dryerr2 = self.fig.add_subplot(self.grid[17:20, 3],
                                                label="Th2")
            ...
            for idx, val in enumerate(t_h):
                self.dryerr2.add_line(Line2D(xdata=[None], ydata=[None],
                                             color=self.cm(self.norm(val)),
                                             linewidth=0, label=val,
                                             marker="o",
                                             markeredgecolor='black',
            ...
            self.drdc2 = self.fig.add_subplot(self.grid[20:27, 3],
                                              label="Th2")
            ...
            for idx, val in enumerate(t_h):
                self.drdc2.add_line(Line2D(xdata=[None], ydata=[None],
                                           color=self.cm(self.norm(val)),
                                           linewidth=0, label=val,
                                           marker="o",
                                           markeredgecolor='black',
            ...
            self.drdcerr2 = self.fig.add_subplot(self.grid[27:30, 3],
                                                 label="Th2")
            self.drdcerr2.set_xlabel("$I_h$ (mA)")
            self.drdcerr2.set_ylabel(r"$\delta$ ($\Omega$)")
            self.drdcerr2.set_xlim(self.xlim_h)
            for idx, val in enumerate(t_h):
                self.drdcerr2.add_line(Line2D(xdata=[None], ydata=[None],
                                              color=self.cm(self.norm(val)),
                                              linewidth=0, label=val,
                                              marker="o",
                                              markeredgecolor='black',
    ...
    class PlotStabilityDiagram:
        ...
        def __init__(self, vg, vb, x, y, z):
            ...
            self.fig.subplots_adjust(top=0.94, bottom=0.085, left=0.065,
                                     right=0.955, hspace=0.19, wspace=0.315)
            ...
            self.ax00.set_title(rf"{z} vs {y}", fontsize=10,
                                fontstyle="italic")
            ...
            self.ax00.ticklabel_format(axis="y", style="scientific",
                                       scilimits=(0, 0))
            for idx, val in enumerate(vg):
                self.ax00.add_line(Line2D(xdata=[None], ydata=[None],
                                          color=self.cm2(self.norm_vg(val)),
                                          linewidth=2, label=val,
                                          alpha=0.4))

            self.ax10 = self.fig.add_subplot(self.grid[1, 0],
                                             sharex=self.ax00)
            self.ax10.set_xlabel(rf"{y}")
            self.ax10.set_ylabel(rf"{z}")
            self.ax10.ticklabel_format(axis="y", style="plain",
                                       scilimits=(0, 0))
            for idx, val in enumerate(vg):
                self.ax10.add_line(Line2D(xdata=[None], ydata=[None],
                                          color=self.cm2(self.norm_vg(val)),
                                          linewidth=2, label=val,
                                          alpha=0.4))

            self.ax01 = self.fig.add_subplot(self.grid[0, 1],
                                             sharey=self.ax00)
            self.ax01.set_title(rf"{z} vs {x}", fontsize=10,
                                fontstyle="italic")
            self.ax01.set_xlabel(rf"{x}")
            self.ax01.set_ylabel(rf"{z}")
            self.ax01.set_xlim([min(vg), max(vg)])
            self.ax01.ticklabel_format(axis="y", style="scientific",
                                       scilimits=(0, 0))
            for idx, val in enumerate(vb):
                self.ax01.add_line(Line2D(xdata=[None], ydata=[None],
                                          color=self.cm2(self.norm_vb(val)),
                                          linewidth=2, label=val,
                                          alpha=0.4))

            self.ax11 = self.fig.add_subplot(self.grid[1, 1],
                                             sharex=self.ax01,
                                             sharey=self.ax10)
            self.ax11.set_xlabel(rf"{x}")
            self.ax11.set_ylabel(rf"{z}")
            self.ax11.ticklabel_format(axis="y", style="plain",
                                       scilimits=(0, 0))
            for idx, val in enumerate(vb):
                self.ax11.add_line(Line2D(xdata=[None], ydata=[None],
                                          color=self.cm2(self.norm_vb(val)),
                                          linewidth=2, label=val,
                                          alpha=0.4))

            self.ax02 = self.fig.add_subplot(self.grid[0, 2:])
            self.ax02.set_title(rf"{z} vs {x} vs {y}", fontsize=10,
                                fontstyle="italic")
            self.ax02.set_xlabel(rf"{x}")
            self.ax02.set_ylabel(rf"{y}")
            self.ax02.set_xlim([min(vg), max(vg)])
            self.ax02.set_ylim([min(vb), max(vb)])
            self.ax02.ticklabel_format(axis="y", style="scientific",
                                       scilimits=(0, 0))
            self.im02 = plt.imshow(temp, aspect="auto", origin="lower",
                                   cmap=self.cm, animated=True,
                                   extent=(
                                       min(vg), max(vg), min(vb), max(vb)))
            ...
            self.ax12.ticklabel_format(axis="y", style="scientific",
                                       scilimits=(0, 0))
            self.im12 = plt.imshow(temp, aspect="auto", origin="lower",
                                   cmap=self.cm, animated=True,
                                   extent=(
                                       min(vg), max(vg), min(vb), max(vb)))
    ...
    class PlotTemperatureVsFrequency:
        ...
        def __init__(self, t, f, i_h, nrows=2, ncols=2, wait=0.001):
            ...
            self.cm = matplotlib.cm.get_cmap("RdYlBu_r")
            self.axlg = [
                Line2D(xdata=[0], ydata=[0], color="grey", marker="o",
                       markersize=6, markeredgecolor='black',
                       markeredgewidth=0.2, linewidth=0, label="Th1"),
                Line2D(xdata=[0], ydata=[0], color="grey", marker="D",
                       markersize=6, markeredgecolor='black',
                       markeredgewidth=0.2, linewidth=0, label="Th2")]
            ...
            for idx, val in enumerate(i_h):
                self.axx.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=f"th1 {val}",
                                         marker="o",
                                         markeredgecolor='black',
                                         markeredgewidth=0.2, alpha=0.4))
                self.axx.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=f"th2 {val}",
                                         marker="D",
                                         markeredgecolor='black',
                                         markeredgewidth=0.2, alpha=0.4))
            ...
            for idx, val in enumerate(i_h):
                self.axy.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=f"th1 {val}",
                                         marker="o",
                                         markeredgecolor='black',
            ...
            for idx, val in enumerate(i_h):
                self.axr.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=f"th1 {val}",
                                         marker="o",
                                         markeredgecolor='black',
            ...
            for idx, val in enumerate(i_h):
                self.axp.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(val)),
                                         linewidth=0, label=f"th1 {val}",
                                         marker="o",
                                         markeredgecolor='black',
    ...
    class PlotDUTVsFrequency:
        ...
        def __init__(self, mode, t, f, i_h, vgs, vds, nrows=1, ncols=2,
                     wait=0.001):
            ...
            if mode == 0:
                self.ax1.set_title(r"$I_{\omega_G}$ vs Frequency",
                                   fontsize=10,
                                   fontstyle="italic")
                self.ax1.set_ylabel(r"$I_{\omega_G}$ (A)")
            elif mode == 1:
                self.ax1.set_title(r"$V_{\omega_G}$ vs Frequency",
                                   fontsize=10,
                                   fontstyle="italic")
            ...
            self.ax2.set_title(r"$I_{2\omega_\alpha}$ vs Frequency",
            ...
            for i in range(len(i_h)):
                for j in range(len(vgs)):
                    for k in range(len(vds)):
                        self.ax1.add_line(
                            Line2D(xdata=[None], ydata=[None], linewidth=0,
                                   marker="o", markeredgecolor='black',
                                   markeredgewidth=0.2,
                                   color=self.cm(self.norm(i_h[i])),
                                   alpha=0.4))
                        self.ax2.add_line(
                            Line2D(xdata=[None], ydata=[None], linewidth=0,
                                   marker="o", markeredgecolor='black',
...
class PlotObsT:
    ...
    def __init__(self, labels, duration, ylabel="Signal (a.u.)",
                 semilogy=False):
        ...
        self.ax.set_title(f"{', '.join(labels)} vs time", fontsize=10,
                          fontstyle="italic")
        ...
        for idx, val in enumerate(labels):
            ...
            self.axlg.append(
                Line2D(xdata=[0], ydata=[0], color=color, marker=marker,
                       markersize=6, markeredgecolor='black',
                       markeredgewidth=0.2, linewidth=0, label=val))
            self.ax.add_line(
                Line2D(xdata=[None], ydata=[None], color=color,
                       marker=marker,
                       markeredgecolor='black', markeredgewidth=0.2,
                       linewidth=0, alpha=0.4, label=val))
...
class FET:
    ...
    class Sweep:
        def __init__(
            ...
            :param sweep: [int] experiment_name, either 0 ("vgs") or 1 ("vds") or 2 ("iv")
        ...
        @staticmethod
        def make_array_4_sweep(params):
            """
            Creates a sweep array based on the input parameters.

            :param params: [list] A list of six elements in the following order:
                           - start (float): The starting value of the sweep.
                           - stop (float): The ending value of the sweep.
                           - steps (int): The number of steps between start and stop.
                           - lin_log (str): 'lin' for linear spacing or 'log' for logarithmic spacing (not yet implemented).
                           - mode (int): The sweep mode:
                               0: Forward sweep only.
                               1: Forward and backward sweep.
                               2: Looping sweep (FWD-BWD-FWD-BWD).
                           - cycles (int): Number of cycles for modes 1 and 2.
            :return: np.array: The generated sweep array.
            """

            # Validate params
            if not isinstance(params, list) or len(params) != 6:
                raise ValueError("The input must be a list with exactly 6 elements.")

            start, stop, steps, lin_log, mode, cycles = params

            # Validate params
            if not isinstance(start, (int, float)) or not isinstance(stop, (int, float)):
                raise ValueError("Start and stop must be numeric values.")
            if not isinstance(steps, int) or steps <= 0:
                raise ValueError("Steps must be a positive integer.")
            if lin_log != 'lin':
                raise NotImplementedError("Only linear spacing ('lin') is currently supported.")
            if mode not in [0, 1, 2]:
                raise ValueError("Mode must be 0 (FWD), 1 (FWD-BWD), or 2 (LOOP).")
            ...
            if mode == 1:
                ...
                y = np.concatenate((y[:-1], np.flip(y), -y[1:-1], np.flip(-y)))

            # Handle cycles
            if mode in [1, 2] and cycles > 1:
                y0 = y[1:]
                y = np.concatenate([y] + [y0] * (cycles - 1))
        ...
        def plot_output_characteristic(self):
            ...
            for i in range(len(self.vgs)):
                ...
                plot0.ax1.set_data(self.data[i, :, 2],
                                   abs(self.data[i, :, 1]))
                plot0.ax1.set_data(self.data[i, :, 2],
                                   abs(self.data[i, :, 3]))
            plt.show()
        ...
        def plot_transfer_characteristic(self):
            ...
            for i in range(len(self.vds)):
                ...
                plot0.ax1.set_data(self.data[i, :, 0],
                                   abs(self.data[i, :, 1]))
                plot0.ax1.set_data(self.data[i, :, 0],
                                   abs(self.data[i, :, 3]))
            plt.show()
    ...
    class SweepVsT:
        def __init__(self, t, vgs, vds):
            ...
            :param sweep: [int] experiment_name, either 0 ("vgs") or 1 ("vds")
    ...
    class PlotOutputCharacteristic:
        ...
        def __init__(self, vgs, vds):
            ...
            self.grid.update(wspace=0.2, hspace=0.2)
            self.cm = matplotlib.cm.get_cmap("RdYlBu_r")
            self.lookIds = {"linewidth": 0, "marker": "o", "markersize": 6,
                            "markeredgecolor": "black",
                            "markeredgewidth": 0.2, "alpha": 0.4}
            self.lookIgs = {"linewidth": 0, "marker": "D", "markersize": 6,
                            "markeredgecolor": "black",
                            "markeredgewidth": 0.2, "alpha": 0.4}
            self.ax0 = self.fig.add_subplot(self.grid[0])
            self.ax0.set_title("Output characteristic", fontsize=10,
                               fontstyle="italic")
            ...
            for i, val in enumerate(vgs):
                self.ax0.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(i)),
                                         **self.lookIgs))
                self.ax0.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(i)),
                                         **self.lookIds))
                self.ax1.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(i)),
                                         **self.lookIgs))
                self.ax1.add_line(Line2D(xdata=[None], ydata=[None],
                                         color=self.cm(self.norm(i)),
                                         **self.lookIds))
            custom_lines = [
                Line2D([0], [0], color=self.cm(self.norm(x)), lw=4)
                for x in range(len(vgs))]
            custom_legend = self.ax0.legend(custom_lines, [x for x in vgs],
                                            title="Vgs", fancybox=True,
                                            framealpha=0.5)
    ...
    class PlotTransferCharacteristic:
        ...
        def __init__(self, vgs, vds):
            ...
            self.lookIgs = {"linewidth": 0, "marker": "D", "markersize": 6,
                            ...
                            "markeredgewidth": 0.2, "alpha": 0.4}
            self.ax0 = self.fig.add_subplot(self.grid[0])
            self.ax0.set_title("Transfer characteristic", fontsize=10,
                               fontstyle="italic")
            ...
            custom_lines = [
                Line2D([0], [0], color=self.cm(self.norm(x)), lw=4)
                for x in range(len(vds))]
            custom_legend = self.ax0.legend(custom_lines, [x for x in vds],
                                            title="Vds", fancybox=True,
                                            framealpha=0.5)
            self.ax0.add_artist(custom_legend)
        ...
        def plot_transfer_characteristic(self, sweep):
            for i in range(len(sweep.vds)):
                ...
                self.ax1.set_data(sweep.data[i, :, 0],
                                  abs(sweep.data[i, :, 1]))
                self.ax1.set_data(sweep.data[i, :, 0],
                                  abs(sweep.data[i, :, 3]))
    ...
    class PlotIV:
        ...
        def __init__(self, vds=array([-1, 1]), xlabel="$V_{ds}$ (V)",
                     ylabel="$I$ (A)", n=1):
            """
            :param vds: [array] Vds (in V)
            :param xlabel: [string] x-axis label
            :param ylabel: [string] y-axis label
            :param n: [int] number of lines. Pass n=0 if adding recursively in the main_path script
            """
            self.grid = GridSpec(2, 1)
            self.fig = plt.figure(figsize=(15 / 2.54, 20 / 2.54))
            self.grid.update(top=0.95, bottom=0.11, left=0.14, right=0.93,
            ...
            for i in range(n):
                self.ax0.add_line(
                    Line2D(xdata=[None], ydata=[None], color="black",
                           **self.lookIds))
                self.ax1.add_line(
                    Line2D(xdata=[None], ydata=[None], color="black",
                           **self.lookIds))
    ...
    class PlotMobility:
        ...
        def __init__(self):
            ...
            self.grid.update(top=0.91, bottom=0.12, left=0.075, right=0.945,
                             hspace=0.2, wspace=0.245)
            self.cm = matplotlib.cm.get_cmap("RdYlBu_r")
            self.lookIds = {"linewidth": 0, "marker": "o", "markersize": 6,
                            "markeredgecolor": "black",
                            "markeredgewidth": 0.2,
            ...
            self.lookIgs = {"linewidth": 0, "marker": "D", "markersize": 6,
                            ...
                            "markeredgewidth": 0.2,
                            "alpha": 0.6}
            self.lookSmooth = {"linewidth": 2, "linestyle": "--",
                               "alpha": 0.6}
            self.ax0 = self.fig.add_subplot(self.grid[0])  # Ids vs Vgs
            self.ax0.set_xlabel("$V_{gs}$ (V)")
            self.ax0.set_ylabel("I (A)")
            self.ax1 = self.fig.add_subplot(
                self.grid[1])  # mobility in the linear region
            self.ax1.set_xlabel("$V_{gs}$ (V)")
    ...
    class PlotContactResistance:
        ...
        def __init__(self):
            ...
            self.grid.update(top=0.91,
                             ...
                             wspace=0.4)
            self.cm = matplotlib.cm.get_cmap("RdYlBu_r")
            self.lookIds = {"linewidth": 0, "marker": "o",
                            "markeredgecolor": "black",
                            "markeredgewidth": 0.2,
                            "alpha": 0.4}
            self.lookIgs = {"linewidth": 0, "marker": "D",
                            "markeredgecolor": "black",
                            "markeredgewidth": 0.2,
                            "alpha": 0.4}
...
class FitDoubleSchottkyBarrier:
    ...
    def func(V, phi01, phi02, T, S1, S2, n1, n2, v1=0.5, v2=0.5):
        ...
        I = 2 * Is1 * Is2 * sinh(V / 2 / beta) / (
                Is1 * exp(V / 2 / beta) + Is2 * exp(- V / 2 / beta))
        return I

    def iv_fit(self, weights=1):
        model = Model(func=self.func,
                      nan_policy="propagate")  # create model object
        # print(f"Parameters: {model.param_names}")
        # print(f"Independent variable: {model.independent_vars}")
        model.set_param_hint('phi01', value=self.phi01_ini,
                             vary=self.phi01_vary, min=self.phi01_min,
                             max=self.phi01_max)  # set parameter phi01 to passed argument
        model.set_param_hint('phi02', value=self.phi02_ini,
                             vary=self.phi02_vary, min=self.phi02_min,
                             max=self.phi02_max)  # set parameter phi02 to passed argument
        model.set_param_hint('T', value=self.T_ini, vary=self.T_vary,
                             min=self.T_min,
                             max=self.T_max)  # set parameter T to passed argument
        model.set_param_hint('S1', value=self.S1_ini, vary=self.S1_vary,
                             min=self.S1_min,
                             max=self.S1_max)  # set parameter S1 to passed argument
        model.set_param_hint('S2', value=self.S2_ini, vary=self.S2_vary,
                             min=self.S2_min,
                             max=self.S2_max)  # set parameter S2 to passed argument
        model.set_param_hint('n1', value=self.n1_ini, vary=self.n1_vary,
                             min=self.n1_min, max=self.n1_max)
        model.set_param_hint('n2', value=self.n2_ini, vary=self.n2_vary,
                             min=self.n2_min, max=self.n2_max)
        model.set_param_hint('v1', value=self.v1_ini, vary=self.v1_vary,
                             min=self.v1_min, max=self.v1_max)
        model.set_param_hint('v2', value=self.v2_ini, vary=self.v2_vary,
                             min=self.v2_min, max=self.v2_max)
    ...
    def recursive_fit(self, model, result):
        param_ini = {"phi01": self.phi01_ini, "phi02": self.phi02_ini,
                     "n1": self.n1_ini, "n2": self.n2_ini,
                     "v1": self.v1_ini,
                     "v2": self.v2_ini}
        param_min = {"phi01": self.phi01_min, "phi02": self.phi02_min,
                     "n1": self.n1_min, "n2": self.n2_min,
                     "v1": self.v1_min,
                     "v2": self.v2_min}
        param_max = {"phi01": self.phi01_max, "phi02": self.phi02_max,
                     "n1": self.n1_max, "n2": self.n2_max,
                     "v1": self.v1_max,
                     "v2": self.v2_max}
        for p in result.params:
            if result.model.param_hints[p]["vary"] is True:
                if result.params[p].stderr == 0 or (
                        result.params[p].stderr > 0 and result.params[
                    p].stderr / result.params[p].value > 0.1) or \
                        result.params[
                            p].stderr is None:
                    result.params[
                        p].stderr = 0  # then set the error to zero
                    print(
                        f"{p} error - re-initializing at {param_min[p] + (param_max[p] - param_min[p]) / self.sweep_steps}")
                    model.set_param_hint(p, value=param_min[p] + (
                            param_max[p] - param_min[p]) / self.sweep_steps,
                                         vary=True, min=param_ini[p],
                                         max=param_max[p])
                    params = model.make_params()  # generate parameter objects
                    print(params["phi01"])
    ...
    class PlotDoubleSchottkyBarrierVsT:
        ...
        def __init__(self, T):
            ...
            self.fig = plt.figure(figsize=(45 / 2.54, 22.5 / 2.54))
            self.grid = GridSpec(nrows=2, ncols=4)
            self.grid.update(top=0.94, bottom=0.085, left=0.065,
                             right=0.955,
                             hspace=0.19, wspace=0.315)
            self.norm_T = matplotlib.colors.Normalize(vmin=min(T),
                                                      vmax=max(T))
            self.cm = matplotlib.cm.get_cmap("RdYlBu_r")
...
class FitPooleFrenkel:
    ...
    def iv_fit(self, weights=1):
        ...
        # print(f"Parameters: {model.param_names}")
        # print(f"Independent variable: {model.independent_vars}")
        model.set_param_hint('phi', value=self.phi_ini, vary=self.phi_vary,
                             min=self.phi_min,
                             max=self.phi_max)  # set parameter phi01 to passed argument
        model.set_param_hint('T', value=self.T_ini, vary=self.T_vary,
                             min=self.T_min,
                             max=self.T_max)  # set parameter T to passed argument
        model.set_param_hint('S', value=self.S_ini, vary=self.S_vary,
                             min=self.S_min,
                             max=self.S_max)  # set parameter S to passed argument
        model.set_param_hint('d', value=self.d_ini, vary=self.d_vary,
                             min=self.d_min, max=self.d_max)
        model.set_param_hint('epsilon_r', value=self.epsilon_r_ini,
                             vary=self.epsilon_r_vary,
                             min=self.epsilon_r_min,
                             max=self.epsilon_r_max)
        model.set_param_hint('sigma0', value=self.sigma0_ini,
                             vary=self.sigma0_vary, min=self.sigma0_min,
                             max=self.sigma0_max)
        params = model.make_params()  # generate parameter objects
        result = model.fit(self.I, params, V=self.V, weights=weights)
        return result
...
class FitSimmons:
    ...
    def simmons_for_intermediate_voltage_range(self):
        """Valid for V > phi/2"""
        model = Model(func=self.simmons,
                      nan_policy="propagate")  # create model object
        # print(f"Parameters: {model.param_names}")
        # print(f"Independent variable: {model.independent_vars}")
        model.set_param_hint("A", value=self.A_ini, vary=self.A_vary,
                             min=self.A_min, max=self.A_max)
        model.set_param_hint("phi", value=self.phi_ini, vary=self.phi_vary,
                             min=self.phi_min, max=self.phi_max)
        model.set_param_hint("d", value=self.d_ini, vary=self.d_vary,
                             min=self.d_min, max=self.d_max)
        params = model.make_params()  # generate parameter objects
        weights = where((self.V <= 2) & (self.V >= 1.5), self.I * 10,
                        self.I)
        result = model.fit(self.I[(abs(self.V) >= 1.5)], params, weights=1,
                           V=self.V[(abs(self.V) >= 1.5)])
        return result
    ...
    def simmons(self, V, A, phi, d):
        ...
        term1 = (phi - e * V / 2) * exp(
            -4 * pi * d / h * sqrt(2 * m_e) * sqrt(phi - e * V / 2))
...
class Figure:
    class PlotLine:
        ...
        def __init__(self, xlabel, ylabel, title=None, obs=None,
                     semilogy=False,
                     sizex=8 / 2.54, sizey=7 / 2.54, cmap=None, norm=None,
                     cmap_label=None, xlim=(-1, 1)):
            """
            :param xlabel: [string] x-axis label
            ...
            if self.cmap is not None and self.norm is not None:
                self.fig.colorbar(
                    mappable=matplotlib.cm.ScalarMappable(norm=self.norm,
                                                          cmap=self.cmap),
                    ax=self.ax, label=cmap_label)
            ...
            if obs is not None:
                for idx, val in enumerate(obs):
                    ...
                    color = matplotlib.cm.Paired(idx)
                    self.axlg.append(
                        Line2D(xdata=[0], ydata=[0], color=color,
                               marker=marker,
                               markersize=6, markeredgecolor='black',
                               markeredgewidth=0.2, linewidth=0, label=val))
                    self.ax.add_line(
                        Line2D(xdata=[None], ydata=[None], color=color,
                               ...
                               markeredgewidth=0.2, linewidth=0, alpha=0.4,
                               label=val))
    ...
    class PlotLineLinAndLog:
        ...
        def __init__(self, xlabel, ylabel, title=None, obs=None,
                     sizex=8 / 2.54,
                     sizey=7 / 2.54, cmap=None, norm=None, cmap_label=None,
                     xlim=(-1, 1)):
            self.fig_lin = Figure.PlotLine(xlabel=xlabel, ylabel=ylabel,
                                           title=title, obs=obs,
                                           semilogy=False,
                                           sizex=sizex, sizey=sizey,
                                           cmap=cmap,
                                           norm=norm, cmap_label=cmap_label,
                                           xlim=xlim)
            self.fig_log = Figure.PlotLine(xlabel=xlabel, ylabel=ylabel,
                                           title=title, obs=obs,
                                           semilogy=True,
                                           sizex=sizex, sizey=sizey,
                                           cmap=cmap,
    ...
    class Plot2D:
        ...
        def __init__(self, x=None, y=None, X=None, title="", xlabel="",
                     ylabel="", interp=None):
            self.grid = GridSpec(1, 1)
            ...
            self.ax.ticklabel_format(axis="y", style="sci",
                                     scilimits=(0, 0))
            self.im = plt.imshow(X, aspect="auto", origin="lower",
                                 cmap=self.cm,
                                 animated=True,
    ...
    class PlotXY:
        ...
        def __init__(self, data, title="", xlabel="", ylabel="",
                     colorcode="index", logx=False):
            self.grid = GridSpec(1, 1)
            self.fig = plt.figure(figsize=(8.5 / 2.54, 6 / 2.54), dpi=300)
            self.grid.update(top=0.9, bottom=0.2, left=0.18, right=0.93)
            self.cm = matplotlib.cm.get_cmap("RdYlBu_r")
            self.norm = Normalize(0, len(data))
            self.lookIds = {"linewidth": 0, "marker": "o", "markersize": 4,
                            "markeredgecolor": "black",
                            "markeredgewidth": 0.2, "alpha": 0.4}
            self.ax = self.fig.add_subplot(self.grid[0])
            self.ax.set_title(title, fontstyle="italic")
            ...
            for idx, val in enumerate(data):
                self.ax.add_line(Line2D(xdata=val[0], ydata=val[1],
                                        color=self.cm(self.norm(idx)),
                                        **self.lookIds, label=val[2]))
            self.ax.legend()
            self.ax.relim()
