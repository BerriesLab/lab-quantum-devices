...
files = sorted(glob.glob(os.path.join("dataset", "msk.nii")))
for file in files:
    ...
    output_file = f"{file[-8]} info.csv"
    # Write the bounding box information to the text file
    with open(output_file, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["bbox enter x", "bbox center y", "bbox center z", "bbox size x", "bbox size y", "bbox size z"])
