...
class BrainLearn:
    ...
    def __init__(self):
        ...
        self._experiment = None  # experiment name. Generated by the generate_experiment_name method launched
    ...
    def generate_experiment_name(self):
        ...
        self._experiment = datetime.datetime.now().strftime('%Y.%m.%d %H.%M.%S')
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for step_tst, batch_tst in enumerate(epoch_tst_iterator):
                ...
                # Run inference by forward passing windowed input data through the model
                prd = sliding_window_inference(img, roi_size=self.patch_size_tst, sw_batch_size=1, predictor=self.model, progress=False)
                # Evaluate metric
                batch_score = self.metric_function(prd, tgt).mean().item()
                # Add batch's validation metric and then calculate average metric
                epoch_score += batch_score
    ...
    def save_dataset_trn_paths_to_csv(self):
        ...
        with open(os.path.join(self.path_main, self._path_model, f"{self._experiment} - dataset trn.csv"), 'w', newline='') as file:
    ...
    def save_dataset_val_paths_to_csv(self):
        """Save list of training data paths to csv."""
        with open(os.path.join(self.path_main, self._path_model, f"{self._experiment} - dataset val.csv"), 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Paths'])  # Writing header
    ...
    def save_dataset_tst_paths_to_csv(self):
        """Save list of training data paths to csv."""
        with open(os.path.join(self.path_main, self._path_model, f"{self._experiment} - dataset tst.csv"), 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Paths'])  # Writing header
    ...
    def save_model(self):
        torch.save(self.model.state_dict(), os.path.join(self.path_main, self._path_model, f"{self._experiment} - iter {self._epoch + 1:03d} mdl.pth"))
    ...
    def save_model_attributes(self):
        ...
        with open(os.path.join(self.path_main, self._path_model, f"{self._experiment} - model attributes.csv"), 'w', newline='') as csvfile:
    ...
    def save_loss(self):
        ...
        np.savetxt(os.path.join(self.path_main, self._path_model, f"{self._experiment} - losses.csv"), data, delimiter=',', header='x,y', comments='')
        plt.plot(data[:, 0], data[:, 1])
        plt.semilogy()
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.savefig(os.path.join(self.path_main, self._path_model, f"{self._experiment} - losses.png"))
        plt.close()

    def save_score(self):
        data = np.column_stack((self.epochs, self.scores))
        np.savetxt(os.path.join(self.path_main, self._path_model, f"{self._experiment} - scores.csv"), data, delimiter=',', header='x,y', comments='')
        data = data[data[0:, 1] != 0]
        plt.plot(data[:, 0], data[:, 1])
        plt.semilogy()
        plt.xlabel("Epoch")
        plt.ylabel("Score")
        plt.savefig(os.path.join(self.path_main, self._path_model, f"{self._experiment} - scores.png"))
        plt.close()
    ...
    def load_model_attributes(self, filename):
