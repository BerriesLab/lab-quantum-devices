...
def register(fix_img: sitk.Image, mov_img: sitk.Image, registration="rigid"):
    ...
    z_fix = np.cross(fix_img_direction_cosines[0, :], fix_img_direction_cosines[1, :])
    z_mov = np.cross(mov_img_direction_cosines[0, :], mov_img_direction_cosines[1, :])
    print(z_fix, z_mov)
    ...
    transform.SetTranslation(fix_img.GetOrigin() - mov_img.GetOrigin())
    sitk.Resample(image1=mov_img,
                  referenceImage=fix_img,
                  transform=transform,
                  default_value=0.0,
                  output_pixel_value=mov_img.GetPixelIDValue())
