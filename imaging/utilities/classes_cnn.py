        ...
        for epoch in range(self.max_iteration_trn):
            ...
            for step_trn, batch_trn in enumerate(epoch_trn_iterator):
                ...
                epoch_trn_iterator.set_description(
                    f"Training ({epoch + 1} / {self.max_iteration_trn} Steps) (loss={epoch_loss:2.5f})")
            ...
            if epoch == 0 or (epoch + 1) % self.delta_iteration_trn == 0 or (epoch + 1) == self.max_iteration_trn:
                ...
                torch.save(self.model, os.path.join(self.path_model,
                                                    f"{self.experiment} - "
                                                    f"iter {epoch + 1:03d} mdl.pth"))
                print("Done.")
        ...
        with torch.no_grad():  # disable gradient computation (which is useless for validation)
            ...
            for step_val, batch_val in enumerate(epoch_val_iterator):
                ...
                epoch_val_iterator.set_description(
                    f"Validate ({epoch + 1} / {self.max_iteration_trn} Steps) (dice={metric_mean:2.5f})")
