...
experiment.experiment_name = "dut frequency response"
        ...
        for idx_vgs, val_vgs in enumerate(vgs):
            ...
            for idx_vds, val_vds in enumerate(vds):
                ...
                for idx_f, val_f in enumerate(f):
                    ...
                    # save figure to disc
                    print("Saving thermalization figure to disc... ", end="")
                    plot2.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment_name} - signals - {val_t:.1f} K - {1e3 * val_i_h:.3f} mA - Vgs {val_vgs:.6f} V - Vds {val_vds:.6f} V - {val_f:.1f} Hz.png", format="png", dpi=300)
                    print("Done.")  # endregion
                ...
                plot1.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment_name} - {val_t:.1f} K.png", format="png", dpi=300)
                print("Done.")
                # endregion
