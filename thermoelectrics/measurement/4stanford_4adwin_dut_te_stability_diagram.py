...
experiment.experiment_name = "dut te stability diagram"
...
settings.adc.iv_settling_time = 0.1         # [float] measurement time (in s). The number of samples is: vt_time / (n_plc / line_freq)
settings.adc.vt_settling_time = 60 * 3      # [float] measurement time (in s). The number of samples is: vt_time / (n_plc / line_freq)
settings.adc.vt_measurement_time = 60 * 1   # [float] measurement time (in s). The number of samples is: vt_time / (n_plc / line_freq)
...
no_samples = int((settings.adc.vt_settling_time + settings.adc.vt_measurement_time) / (settings.adc.n_plc / settings.adc.line_freq))
no_samples2avg = int(settings.adc.vt_measurement_time / (settings.adc.n_plc / settings.adc.line_freq))
    ...
    for idx_i_h, val_i_h in enumerate(i_h):
        ...
        for idx_vgs, val_vgs in enumerate(vgs):
            ...
            for idx_vds, val_vds in enumerate(vds):
                ...
                # save figure to disc
                print("Saving thermalization figure to disc... ", end="")
                plot4.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment_name} - signals - {val_t:.1f} K - {1e3 * val_i_h:.3f} mA - Vgs {val_vgs:.6f} V - Vds {val_vds:.6f} V.png", format="png", dpi=300)
                print("Done.")  # endregion
            ...
            if mode == 0:
                plot1.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment_name} - IwG - {val_t:.1f} K - {1e3 * val_i_h:.1f} mA.png", format="png", dpi=300)
                plot2.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment_name} - I2wA - {val_t:.1f} K - {1e3 * val_i_h:.1f} mA.png", format="png", dpi=300)
            elif mode == 1:
                plot1.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment_name} - VwG - {val_t:.1f} K - {1e3 * val_i_h:.1f} mA.png", format="png", dpi=300)
                plot2.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment_name} - I2wA - {val_t:.1f} K - {1e3 * val_i_h:.1f} mA.png", format="png", dpi=300)
            print("Done.")
            # endregion
