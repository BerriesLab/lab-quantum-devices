from numpy import array, where, zeros, sqrt, linspace, sinh, exp, concatenate, flip, ceil, nan, zeros_like, ones_like, empty, delete, unique
...
class Thermoelectrics:
    ...
    class PlotStabilityDiagram:
        ...
        def __init__(self, vg, vb, x, y, z):
            ...
            self.ax02.set_title(rf"{z} vs {x} vs {y}")
...
class FET:
    ...
    class Sweep:
        ...
        def filter_vds_fwd_sweep(self):
            ...
            for j in range(self.data.shape[1]):
                if j == 0:
                    ...
                    if all(self.data[:, j, 0] - self.data[:, j-1, 0] > 0):
                        if k == 0:
                            newdata[:, k, :] = self.data[:, j-1, :]
        ...
        def filter_vgs_values(self, values):
            ...
            k = 0
            for i in range(self.data.shape[0]):
                if all(self.data[i, :, 0] in values):
                    newdata[k, :, :] = self.data[i, :, :]
                    k = k + 1
            return newdata[0:k, :, :]
    ...
    class FETvsT:
        def __init__(self, t, vgs, vds):
            ...
            """
            self.data = {}
            self.data["vgs"] = FET.SweepVsT(t, vgs, vds)
            self.data["vds"] = FET.SweepVsT(t, vgs, vds)
    ...
    class PlotOutputCharacteristic:
        ...
        def plot_output_characteristic(self, sweep):
            for i in range(len(sweep.vgs)):
                self.ax0.set_data(sweep.data[i, :, 2], sweep.data[i, :, 1])
                self.ax0.set_data(sweep.data[i, :, 2], sweep.data[i, :, 3])
                self.ax1.set_data(sweep.data[i, :, 2], abs(sweep.data[i, :, 1]))
                self.ax1.set_data(sweep.data[i, :, 2], abs(sweep.data[i, :, 3]))
    ...
    class PlotIV:
        ...
        def plot_iv(self, sweep):
            self.ax0.set_data(sweep.data[0, :, 2], sweep.data[0, :, 3])
            self.ax1.set_data(sweep.data[0, :, 2], abs(sweep.data[0, :, 3]))
