...
fig2, ax2= plt.subplots(1, 3, figsize=(16 / 2.54, 6 / 2.54), dpi=300)
...
ax2[0].set_xlabel("Device area ($m^{-2}$)")
ax2[0].set_ylabel("Series resistance ($\Omega$)")
ax2[0].ticklabel_format(axis='y', style='sci', scilimits=(0, 0))

ax2[1].set_xlabel("Device area ($m^{-2}$)")
ax2[1].set_ylabel("Relative permittivity")
ax2[1].ticklabel_format(axis='y', style='sci', scilimits=(0, 0))

ax2[2].set_xlabel("Interface")
ax2[2].set_ylabel("Potential barrier ($eV$)")
ax2[2].ticklabel_format(axis='y', style='sci', scilimits=(0, 0))

Rss = np.arange(0, 20.001e4, 10)
...
for x in data2load:
    ...
    for j, y in enumerate(x[1]):
        ...
        for idxf, z in enumerate(files):
            ...
            device_area_pos = np.pi * ((device_dic[y[0]]) * 1e-6 / 2) ** 2
            device_area_neg = np.pi * ((device_dic[y[0]] + 2) * 1e-6 / 2) ** 2
            ...
            # Current density
            voltage = xdata
            current_density = np.zeros(len(xdata))
            current_density[voltage >= 0] = ydata[voltage >= 0] / device_area_pos
            current_density[voltage < 0] = ydata[voltage < 0] / device_area_neg
            ...
            for idx, val in enumerate(Rss):
                rs_voltage = np.zeros(len(voltage))
                rs_voltage[voltage >= 0] = np.sqrt(np.abs(voltage[voltage >= 0] - val * current_density[voltage >= 0] * device_area_pos)) * np.sign(
                    voltage[voltage >= 0] - val * current_density[voltage >= 0] * device_area_pos)
                rs_voltage[voltage < 0] = np.sqrt(np.abs(voltage[voltage < 0] - val * current_density[voltage < 0] * device_area_neg)) * np.sign(
                    voltage[voltage < 0] - val * current_density[voltage < 0] * device_area_neg)
                # rs_voltage = voltage
            ...
            # Plot Schottky
            Rs = fit_results[2]
            rs_voltage[voltage >= 0] = np.sqrt(
                np.abs(voltage[voltage >= 0] - Rs * current_density[voltage >= 0] * device_area_pos)) * np.sign(
                voltage[voltage >= 0] - Rs * current_density[voltage >= 0] * device_area_pos)
            rs_voltage[voltage < 0] = np.sqrt(
                np.abs(voltage[voltage < 0] - Rs * current_density[voltage < 0] * device_area_pos)) * np.sign(
                voltage[voltage < 0] - Rs * current_density[voltage < 0] * device_area_neg)
            ln_current_density = np.log(np.abs(current_density))
            ...
            pos_theor_current_density = pos_richardson * T ** 2 * np.exp(-e * (pos_potential_barrier - np.sqrt(e * np.abs(voltage[voltage >= 0] - Rs * current_density[voltage >= 0] * device_area_pos) / (thickness * 4 * np.pi * vac_perm * perm_r))) / (k * T))
            neg_theor_current_density = -neg_richardson * T ** 2 * np.exp(-e * (neg_potential_barrier - np.sqrt(e * np.abs(voltage[voltage <= 0] - Rs * current_density[voltage <= 0] * device_area_neg) / (thickness * 4 * np.pi * vac_perm * perm_r))) / (k * T))
...
ax2[0].semilogx(results[:, 1], results[:, 2], 'o', alpha=0.5)
ax2[1].semilogx(results[:, 1], results[:, 5], 'o', alpha=0.5)
ax2[2].boxplot([results[:, 6], results[:, 7]], labels=["Gr/P3HT", "Au/P3HT"])

fig1.tight_layout()  # adjust only when called, just before saving
fig2.tight_layout()  # adjust only when called, just before saving
...
fig2.savefig(fname=fig_path + r'\thermionic_stat.png', **fig_settings)
