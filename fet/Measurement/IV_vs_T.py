...
experiment.main_path = r"C:\samples\dabe"
...
if annealing[0] is not None and settings.tc.address is not None:
    ...
    plot0.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment_name} - annealing - {annealing[0]:.1f} K.png", format="png", dpi=300)
    print("Done.")
