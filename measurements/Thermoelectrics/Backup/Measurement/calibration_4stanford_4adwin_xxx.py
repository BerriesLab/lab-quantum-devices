...
for idx_t, val_t in enumerate(t):
    ...
    if thermometer == 1:
        ...
        adc.adw.SetData_Long(list(adc.voltage2bin(zeros(len(i_th)) / settings.src2.gain, bits=settings.adc.output_resolution)), 22, 1, len(i_th))
        adc.adw.Set_Par(42, len(i_th))  # set length of v2 array

    if thermometer == 2:
        adc.adw.SetData_Long(list(adc.voltage2bin(zeros(len(i_th)) / settings.src1.gain, bits=settings.adc.output_resolution)), 21, 1, len(i_th))
        adc.adw.Set_Par(41, len(i_th))  # set length of v1 array
    ...
    if val_t in t_h:
        ...
        for idx_i_h, val_i_h in enumerate(i_h):
            ...
            if thermometer == 1:
                ...
                adc.adw.Set_Par(42, len(temp))  # set length of v2 array
                adc.adw.SetData_Long(list(adc.voltage2bin(zeros(len(temp)) / settings.src2.gain, bits=settings.adc.output_resolution)), 22, 1, len(temp))  # set ao2 data to 0 V

            if thermometer == 2:
                adc.adw.Set_Par(41, len(temp))  # set length of v1 array
                adc.adw.SetData_Long(list(adc.voltage2bin(zeros(len(temp)) / settings.src1.gain, bits=settings.adc.output_resolution)), 21, 1, len(temp))  # set ao1 data to 0 V
                adc.adw.Set_Par(42, len(temp))  # set length of v2 array
            ...
            if thermometer == 1:
                ...
                adc.adw.Set_Par(41, len(temp))  # set length of v1 array
                adc.adw.SetData_Long(list(adc.voltage2bin(zeros(len(temp)) / settings.src2.gain)), 22, 1, len(temp))  # set ao2 data
                adc.adw.Set_Par(42, len(temp))  # set length of v2 array

            if thermometer == 2:
                adc.adw.SetData_Long(list(adc.voltage2bin(zeros(len(temp)) / settings.src1.gain)), 21, 1, len(temp))  # set ao1 data
                adc.adw.Set_Par(41, len(temp))  # set length of v1 array
