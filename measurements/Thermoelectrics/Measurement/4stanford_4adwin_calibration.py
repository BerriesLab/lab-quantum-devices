...
for idx_t, val_t in enumerate(t):
    ...
    if settings.tc.address is not None:
        ...
        plot2.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment} - thermalization - {val_t:.1f} K.png", format="png", dpi=300)
        print("Done.")
    ...
    if val_t in t_h:
        ...
        for idx_i_h, val_i_h in enumerate(i_h):
            ...
            # number of samples of a time-dependent measurement = time / (nplc * line_freq)
            adc.adw.Set_Par(41, int(ceil((settings.adc.vt_settling_time + settings.adc.vt_measurement_time) / (settings.adc.nplc / settings.adc.line_freq))))
            ...
            plot3.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment} - oscillations - {val_t:.1f} K - {val_i_h*1000:.1f} mA.png", format="png", dpi=300)
            print("Done.")
