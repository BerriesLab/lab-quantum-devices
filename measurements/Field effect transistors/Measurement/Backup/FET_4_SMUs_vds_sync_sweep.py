...
vgs = np.array([0])            # [array] Vgs (in V).
vds = [0, 20, 0.1, "lin", 2]             # [list] Vds (in V). In the order: start, stop, step size, lin-log, mode (0: single, 1: return, 2: hysteresis)
# smu gate options --------------------------------------------------------------------------------------------------
...
smu1_delay_init = 10000                 # [float] time (in ms) between first source and measure cycle
# smu vds options --------------------------------------------------------------------------------------------
...
smu2_delay = 100                        # [float] time (in ms) between source and measure.
smu2_delay_init = 100                    # [float] time (in ms) between first source and measure cycle
...
n_vgs = len(vgs)
n_vds = int((vds[1] - vds[0]) / vds[2] + 1)
if vds[4] == 1:
    n_vds = 2 * n_vds - 1
if vds[4] == 2:
    n_vds = 4 * n_vds - 3
if n_vds > 1000:
    exit("Number of data points to store in SMU unit > 1000. Terminating program.")
...
print("***** Measurement summary *****\n"
      "Vds sweep from: {} V to: {} V with step: {} V, type: {} and mode: {}\n"
      "Vgs values {}".format(vds[0], vds[1], vds[2], vds[3], vds[4], vgs))
if default_delay1 is not None and default_delay2 is not None:
    est_time = n_vgs * (smu1_delay_init/1000 + n_vds * (default_delay1 + smu1_delay / 1000 + smu1_integration_time * n1 +
                                                   default_delay2 + smu2_delay / 1000 + smu2_integration_time * n2))
    print("Estimated measurement time: {:.1f} min.\n".format(est_time / 60))
else:
    est_time = n_vgs * (smu1_delay_init/1000 + n_vds * (smu1_delay / 1000 + smu1_integration_time * n1 +
...
file_name = "{} - 3 terminal fet vds sweep.bin".format(date.strftime("%Y.%m.%d %H.%M.%S"))
file_backup_name = file_name + ".bak"
fig_name = "{} - 3 terminal fet vds sweep.png".format(date.strftime("%Y.%m.%d %H.%M.%S"))
...
ax1_lin.set_ylabel("$I_{ds}$ (A)", color="orangered")
...
norm_vg = matplotlib.colors.Normalize(vmin=vgs.min(), vmax=vgs.max())
print("Done.")
# endregion

# region ----- Configuring instrumentation -----
...
smu1.set_trigger_control("external", "^src dly msr", "src dly msr^", "enabled")
smu1.set_trigger_on()
smu2.set_source("v")
...
smu2.set_trigger_control("external", "^src dly msr", "src dly msr^", "disabled")
smu2.set_trigger_on()
print("Done.")
# endregion
...
for idx, val in enumerate(vgs):

    print("Making Vds sweep at Vg = {} ... ".format(val), end="")

    smu1.create_fixed_staircase(val, smu1_source_range, smu1_delay_init, 1)
    smu1.append_fixed_staircase(val, smu1_source_range, smu1_delay, n_vds-1)

    if vds[3] == "lin":
        if vds[4] == 0:
            smu2.create_linear_staircase(vds[0], vds[1], vds[2], smu2_source_range, smu2_delay)
        if vds[4] == 1:
            smu2.create_linear_staircase(vds[0], vds[1], vds[2], smu2_source_range, smu2_delay)
            smu2.append_linear_staircase(vds[1] - vds[2], vds[0], vds[2], smu2_source_range, smu2_delay)
        if vds[4] == 2:
            smu2.create_linear_staircase(vds[0], vds[1], vds[2], smu2_source_range, smu2_delay)
            smu2.append_linear_staircase(vds[1] - vds[2], vds[1] - 2 * (vds[1] - vds[0]), vds[2], smu2_source_range, smu2_delay)
            smu2.append_linear_staircase(vds[1] - 2 * (vds[1] - vds[0]) + vds[2], vds[0], vds[2], smu2_source_range, smu2_delay)
    if vds[3] == "log":
        print("Not yet implemented")
        if vds[4] == 0:
            smu2.create_logarithmic_staircase(vds[0], vds[1], vds[2], smu2_source_range, smu2_delay)
        if vds[4] == 1:
            smu2.create_logarithmic_staircase(vds[0], vds[1], vds[2], smu2_source_range, smu2_delay)
            smu2.append_logarithmic_staircase(vds[1], vds[0], vds[2], smu2_source_range, smu2_delay)
        if vds[4] == 2:
            smu2.create_logarithmic_staircase(vds[0], vds[1], vds[2], smu2_source_range, smu2_delay)
            smu2.append_logarithmic_staircase(vds[1], vds[1] - 2 * (vds[1] - vds[0]), vds[2], smu2_source_range, smu2_delay)
            smu2.append_logarithmic_staircase(vds[1] - 2 * (vds[1] - vds[0]), vds[0], vds[2], smu2_source_range, smu2_delay)
    ...
    ax1_lin.plot(data[idx, :, 0], data[idx, :, 1], linewidth=0, marker="o", markeredgecolor="black", markerfacecolor=cmap1(norm_vg(val)) if len(vgs) > 1 else "orange", alpha=0.4)
    ax2_lin.plot(data[idx, :, 0], data[idx, :, 3], linewidth=0, marker="D", markeredgecolor="black", markerfacecolor=cmap2(norm_vg(val)) if len(vgs) > 1 else "green", alpha=0.4)
    ax3_log.semilogy(data[idx, :, 0], abs(data[idx, :, 1]), linewidth=0, marker="o", markeredgecolor="black", markerfacecolor=cmap1(norm_vg(val)) if len(vgs) > 1 else "orange", alpha=0.4)
    ax3_log.semilogy(data[idx, :, 0], abs(data[idx, :, 3]), linewidth=0, marker="D", markeredgecolor="black", markerfacecolor=cmap2(norm_vg(val)) if len(vgs) > 1 else "green", alpha=0.4)
    plt.pause(0.0001)
    print("Done.")
    # endregion
    ...
    with open(file_name, "wb") as file:
        pickle.dump({"date": date,
                     ...
                     "experiment": "3 terminal fet vds sweep",
