...
import os
from numpy import linspace, savetxt, floor, log10
import time
import datetime
import pickle
# endregion
...
experiment.main = r"C:\samples\osja"
experiment.date = datetime.datetime.now()
experiment.chip = "au p3ht fet 01"
experiment.device = "idg a-5um"
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------
vgs = [40, -40, 5, "lin", 0, 1]         # [list] Vgs (in V). In the order: start, stop, steps, lin-log, mode (0: FWD, 1: FWD-BWD, 2: LOOP), cycles (>0)
vds = [0, 50, 101, "lin", 2, 5]           # [list] Vds (in V). In the order: start, stop, steps, lin-log, mode (0: FWD, 1: FWD-BWD, 2: LOOP), cycles (>0)
fet = FET.Sweep(sweep, vgs, vds)  # ---------------------------------------------------------------------
fet.environment = 0                     # [int] measurement environment {0: vacuum, 1: air, 2: N2, 3: Ar}
fet.illumination = 0                    # [int] illumination {0: dark, 1: light}
fet.temperature = 273 + 25              # [float] temperature (in K)
fet.channel_length = 5e-6               # [float] channel length (in m)
fet.channel_width = 1e-3                # [float] channel length (in m)
...
settings.smu_vgs.delay = 0.0                    # [float] time (in s) between source and measure
settings.smu_vgs.delay_init = 10                 # [float] time (in s) between source and measure
settings.smu_vgs.ramp_step = 100e-3             # [float] gate sweep step size (in V) (when sweeping Vds)
settings.smu_vgs.ramp_delay = 10e-2             # [float] gate sweep step size (in V) (when sweeping Vds)
# -------------------------------------------------------------------------------------------------
...
settings.smu_vds.source_range = 110             # [float] gate voltage source range
settings.smu_vds.sense_range = 1e-3           # [float] current sense range
...
settings.smu_vds.delay = 0.0                    # [float] time (in s) between source and measure
settings.smu_vds.delay_init = 10                 # [float] time (in s) between source and measure
settings.smu_vds.ramp_step = 100e-3             # [float] drain-source sweep step size (in V) (when sweeping Vgs)
settings.smu_vds.ramp_delay = 10e-2             # [float] drain-source sweep step size (in V) (when sweeping Vgs)
# -------------------------------------------------------------------------------------------------
...
if sweep == 0 or sweep == 1:
    smu_vgs.bias("v", 0, settings.smu_vgs.source_range, settings.smu_vgs.sense_range, settings.smu_vgs.delay,
                 settings.smu_vgs.samples, settings.smu_vgs.integration_time, settings.smu_vgs.sensing, settings.smu_vgs.compliance)
smu_vds.bias("v", 0, settings.smu_vds.source_range, settings.smu_vds.sense_range, settings.smu_vds.delay,
             settings.smu_vds.samples, settings.smu_vds.integration_time, settings.smu_vds.sensing, settings.smu_vds.compliance)
# endregion
...
input(f"Vds total delay time: {'> ' if settings.smu_vds.sense_range == 'auto' else ''}{(vds_default_delay + settings.smu_vds.delay) * 1e3:.0f} ms\n"
...
path = rf"{experiment.main}\{experiment.chip}\{experiment.device}\fet\{subdir[sweep]}"
try:
...
plot1 = FET.PlotFET(vgs=fet.vgs, vds=fet.vds, sweep=sweep)
...
if sweep == 0:
    ...
    for i in range(len(fet.vds)):
        ...
        if i == 0 and fet.vds[0] == 0:
            ...
            for x in linspace(0, fet.vds[i], int(abs(fet.vds[i]) / settings.smu_vds.ramp_step)):
                smu_vds.set_bias_level(bias=x, delay=settings.smu_vds.ramp_delay)
        else:
            for x in linspace(fet.vds[i-1], fet.vds[i], int(abs(fet.vds[i] - fet.vds[i-1]) / settings.smu_vds.ramp_step)):
                smu_vds.set_bias_level(bias=x, delay=settings.smu_vds.ramp_delay)
        print("Done.")
        # endregion
        ...
        for j in range(len(fet.vgs)):
            ...
            if i == 0 and j == 0 and fet.vgs[0] != 0:
                ...
                for x in linspace(fet.vgs[-1], fet.vgs[0], int(abs(fet.vgs[-1] - fet.vgs[0]) / settings.smu_vgs.ramp_step) + 1):
                    smu_vgs.set_bias_level(bias=x, delay=settings.smu_vgs.ramp_delay)
            else:
        ...
        if os.path.exists(experiment.filename[:-4] + "txt"):
            if os.path.exists(experiment.backupname[:-8] + "txt.bak"):
                os.remove(experiment.backupname[:-8] + "txt.bak")
            os.rename(experiment.filename[:-4] + "txt", experiment.backupname[:-8] + "txt.bak")
        txt = []
        for k in range(fet.data.shape[1]):
            for q in range(fet.data.shape[0]):
                txt.append(fet.data[q, k, :])
        savetxt(experiment.filename[:-4] + "txt", txt, delimiter=",", newline='\n', header="vgs,igs,vds,ids,cycle,time", comments="", footer='', encoding=None)
        print("Done.")  # endregion

    # region ----- Set Vds to 0 V -----
    print("Sweeping Vds from {} V to 0 V... ".format(fet.vds[-1]), end="")
    for x in linspace(fet.vds[-1], 0, int(abs(fet.vds[-1]) / settings.smu_vds.ramp_step)):
        smu_vds.set_bias_level(bias=x, delay=settings.smu_vds.ramp_delay)
    print("Done.")  # endregion

    # region ----- Set Vgs to 0 V -----
    print("Sweeping Vgs from {} V to 0 V... ".format(fet.vgs[-1]), end="")
    for x in linspace(fet.vgs[-1], 0, int(abs(fet.vgs[-1]) / settings.smu_vgs.ramp_step)):
        smu_vgs.set_bias_level(bias=x, delay=settings.smu_vgs.ramp_delay)
    print("Done.")  # endregion
...
if sweep == 1:
    ...
    for i in range(len(fet.vgs)):
        ...
        if i == 0 and fet.vgs[0] == 0:
            ...
            for x in linspace(0, fet.vgs[i], int(abs(fet.vgs[i]) / settings.smu_vgs.ramp_step) + 1):
                smu_vgs.set_bias_level(bias=x, delay=settings.smu_vgs.ramp_delay)
        else:
            for x in linspace(fet.vgs[i-1], fet.vgs[i], int(abs(fet.vgs[i] - fet.vgs[i-1]) / settings.smu_vgs.ramp_step) + 1):
                smu_vgs.set_bias_level(bias=x, delay=settings.smu_vgs.ramp_delay)
        print("Done.")
        # endregion
        ...
        for j in range(len(fet.vds)):
            ...
            if i == 0 and j == 0 and fet.vds[0] != 0:
                ...
                for x in linspace(fet.vds[-1], fet.vds[0], int(abs(fet.vds[-1] - fet.vds[0]) / settings.smu_vds.ramp_step) + 1):
                    smu_vds.set_bias_level(bias=x, delay=settings.smu_vds.ramp_delay)
            else:
        ...
        for k in range(fet.data.shape[1]):
            for q in range(fet.data.shape[0]):
                txt.append(fet.data[q, k, :])
        savetxt(experiment.filename[:-4] + "txt", txt, delimiter=",", newline='\n', header="vgs,igs,vds,ids,vgs cycle,vds cycle, time", comments="", footer='', encoding=None)
        print("Done.")  # endregion
...
if sweep == 2:
    ...
    for j in range(len(fet.vds)):
        ...
        # region ----- Update figure -----
        plot1.ax0.lines[2 * 0 + 1].set_data(fet.data[0, 0: j + 1, 2], fet.data[0, 0: j + 1, 3])
        plot1.ax0.relim()
        plot1.ax0.autoscale_view(scalex=False, scaley=True)
        plot1.ax1.lines[2 * 0 + 1].set_data(fet.data[0, 0: j + 1, 2], log10(abs(fet.data[0, 0: j + 1, 3])))
        plot1.ax1.relim()
        plot1.ax1.autoscale_view(scalex=False, scaley=True)
        plt.pause(1e-3)
        # endregion
