...
df["p(NH3):p(H2)"] = df["p(NH3)"]/df["p(amu 2)"]

#Calculate averages
window = 30
df_avg = df.loc[(df["Time"]<df["Time notes"])&(df["Time"]>(df["Time notes"]-window))]
df_avg = df.groupby(["Pressure","N2","H2","NH3"], as_index=False).mean()
df_avg = df_avg[["Pressure","NH3:N2","NH3:H2","p(NH3):p(H2)"]]
df_avg["delta_beta"] = df_avg["NH3:H2"]-df_avg["p(NH3):p(H2)"]

#PLOTTING
fig, ax = plt.subplots()
df_avg.groupby("Pressure").plot(x="p(NH3):p(H2)",y="delta_beta", ax=ax, marker="o", linewidth=0, logx=False)
legend = df_avg.groupby("Pressure").groups.keys()
plt.legend([str(x)+" Torr" for x in legend])
ax.set_xlabel("$\\beta$")
ax.set_ylabel("$\Delta\\beta = B - \\beta$")
plt.title("$\Delta\\beta(\\beta, P)$")

df_fit = pandas.DataFrame(columns=["Pressure","theta_0","theta_1","theta_2", "std"])
for index in df_avg["Pressure"].unique():
    x = df_avg[(df_avg["Pressure"]==index)]["p(NH3):p(H2)"].values
    y = df_avg[(df_avg["Pressure"]==index)]["delta_beta"].values
...
text = r"Model: $\Delta\beta(\beta,P) = \Theta_{0,P}^{(\beta)} + \Theta_{1,P}^{(\beta)}\beta +\Theta_{2,P}^{(\beta)}\beta^2$"
ax.text(0.05, 0.97, text, transform=ax.transAxes, fontsize=10, verticalalignment='top')
plt.show()

print(df_fit)
#SAVE file
df_fit.to_csv(file_dir+"betas.csv", index=False)