...
EVENT:
  ...
  SELECTCASE flag '0 = ramp to next voltage point ; 1 = wait ; 2 = measure 
    ...
    CASE 2 'measure --------------------------------
      ...
      IF (PAR_21 = 1) THEN sum1 = sum1 + READ_ADC24(1)/64
      IF (PAR_22 = 1) THEN sum2 = sum2 + READ_ADC24(2)/64
      ...
      IF(idx_avg = PAR_33) THEN 
        'get average values
        IF ((PAR_21 = 1) AND (idx_scan <= PAR_41)) THEN 
          DATA_11[idx_scan] = sum1 / PAR_33 
          sum1 = 0
        ENDIF       
        IF ((PAR_22 = 1) AND (idx_scan <= PAR_42)) THEN 
