...
INIT:
  ...
  v = DATA_22[1]
  DAC(2, v)  'set output to first value
  PAR_51 = v
  ...
  SELECTCASE flag '0 = ramp to next voltage point ; 1 = wait ; 2 = measure 
    ...
    CASE 0 'set analog output --------------------------------
      ...
      IF (idx_scan <= PAR_41) THEN
        IF (v < DATA_22[idx_scan]) THEN INC(v)      
        IF (v > DATA_22[idx_scan]) THEN  DEC(v)
        IF (v = (DATA_22[idx_scan])) THEN flag = 1
        PAR_51 = v
        DAC(2, v)
      ENDIF        
    ...
    CASE 2 'measure --------------------------------
      ...
      'read data
      START_CONV(10b)  'start conversion on ADC 2
      WAIT_EOC(10b)  'wit end of conversion on ADC 2
          
      sum = sum + READ_ADC24(1)/64
      idx_avg = idx_avg + 1
        
      IF(idx_avg = PAR_33) THEN  'if reached number of samples to average
        DATA_2[idx_scan] = sum / PAR_33  'average values
