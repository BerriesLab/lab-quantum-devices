    ...
    for idx_i_h, val_i_h in enumerate(i_h):
        ...
        for idx_vg, val_vg in enumerate(vg):
            ...
            for idx_vb, val_vb in enumerate(vb):
                ...
                print(f"Setting DC bias from {val_vb_:.1f} V to {val_vb:.3f} V... ", end="")
                adc.adw.Set_Par(21, 0)  # set ao1 OFF
                adc.adw.Set_Par(22, 1)  # set ao2 ON

                if val_vb_ == val_vb:
                    temp_vb = array([val_vb])
                else:
                    temp_vb = linspace(0 if idx_vb == 0 else vb[idx_vb - 1], vb[idx_vb], vb_steps)
