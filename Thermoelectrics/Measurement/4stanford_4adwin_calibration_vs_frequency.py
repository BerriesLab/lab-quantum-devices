...
experiment.experiment = "calibration vs frequency"
...
for idx_t, val_t in enumerate(t):
    ...
    for idx_i_h, val_i_h in enumerate(i_h):
        ...
        for idx_f, val_f in enumerate(f):
            ...
            plot2.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment} - oscillations - {val_t:.1f} K - {val_i_h * 1000:.1f} mA - {val_f:.1f} Hz.png", format="png", dpi=300)
            print("Done.")
            # endregion
    ...
    plot1.fig.savefig(fname=f"{experiment.date.strftime('%Y-%m-%d %H.%M.%S')} - {experiment.chip} - {experiment.device} - {experiment.experiment} - {val_t:04.1f} K - {1e3*val_i_h:04.1f} mA - {f[0]:.1f} Hz - {f[-1]:04.1f} Hz.png", format="png", dpi=300)
    print("Done.")
    # endregion
