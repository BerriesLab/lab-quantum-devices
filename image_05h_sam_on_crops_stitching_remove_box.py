...
for c, model in enumerate(MODEL):
    for r, point_per_side in enumerate(POINTS_PER_SIDE):
        ...
        for k, image in enumerate(IMAGE):
            ...
            plt.title(f"Number of segments: {int(len(unique(labels)))}")
            plt.tight_layout()
            plt.savefig(rf"{OUTPUT_PATH}\segments with box filtered,{image:02d}.jpg,{model},{point_per_side}.png", dpi=1200, bbox_inches='tight')

            plt.ylim(400, 601)
            plt.savefig(rf"{OUTPUT_PATH}\segments with box filtered crop,{image:02d}.jpg,{model},{point_per_side}.png", dpi=1200, bbox_inches='tight')
            plt.close()
